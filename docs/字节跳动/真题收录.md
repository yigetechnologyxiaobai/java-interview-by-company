# 字节跳动 - 真题收录

> 本文档整理自 字节跳动 面经真题，按知识点归类

## 目录

- [2026 Java 面经真题](#2026-java-面经真题)
- [2026 最新面经](#2026-最新面经)

---

## 2026 Java 面经真题


> 来源：牛客网 2026-05
> 特点：两面连着面，项目 + 八股 + 算法

### 一面真题

#### Q1: 让你实现一个 HashMap，你会如何设计？

**参考答案**：

1. **核心数据结构**：
   - 数组 + 链表 + 红黑树（JDK 1.8+）
   - 初始容量 16，负载因子 0.75

2. **哈希函数**：
   ```java
   int hash = key.hashCode() ^ (key.hashCode() >>> 16);  // 扰动函数
   int index = (n - 1) & hash;  // 位运算取模
   ```

3. **冲突解决**：
   - 链地址法
   - 链表长度 ≥ 8 且数组长度 ≥ 64 时转红黑树

4. **扩容机制**：
   - 当 size > threshold 时扩容 2 倍
   - 重新计算元素位置（高位/低位分流）

---

#### Q2: synchronized 性能为什么提高了？

**参考答案**：锁升级机制。

```
无锁 → 偏向锁 → 轻量级锁 → 重量级锁
```

| 锁状态 | 适用场景 | 性能 |
|--------|---------|------|
| 偏向锁 | 单线程访问 | 最高 |
| 轻量级锁 | 交替执行 | 高 |
| 重量级锁 | 竞争激烈 | 低 |

**升级过程**：
1. 偏向锁：对象头记录线程 ID
2. 轻量级锁：CAS 自旋尝试获取
3. 重量级锁：阻塞等待，OS 互斥量

---

#### Q3: HTTP 报文结构？头部有哪些字段？

**请求报文**：
```
POST /api/user HTTP/1.1          // 请求行
Host: www.example.com            // 请求头
Content-Type: application/json
Content-Length: 25

{"name":"Tom","age":20}          // 请求体
```

**常见请求头**：
- Host：目标主机
- Content-Type：请求体类型
- Authorization：认证信息
- User-Agent：客户端信息
- Accept：接受的内容类型

**常见响应头**：
- Content-Type：响应体类型
- Set-Cookie：设置 Cookie
- Cache-Control：缓存控制
- Location：重定向地址

---

#### Q4: TCP 三次握手，TIME_WAIT 和 CLOSE_WAIT 的作用？

**TIME_WAIT**：
- 主动关闭方进入
- 等待 2MSL（Maximum Segment Lifetime）
- 作用：确保最后的 ACK 到达对方

**CLOSE_WAIT**：
- 被动关闭方进入
- 等待应用层调用 close()
- 大量 CLOSE_WAIT 说明代码没正确关闭连接

**为什么是三次握手？**
- 两次可能导致：失效的 SYN 到达服务端，服务端误建立连接
- 三次确保双方都能确认对方的接收和发送能力

---

#### Q5: 算法题 - 表达式求值（+ - * /）

**参考解答**：
```java
public int calculate(String s) {
    Stack<Integer> stack = new Stack<>();
    int num = 0;
    char sign = '+';
    
    for (int i = 0; i < s.length(); i++) {
        char c = s.charAt(i);
        if (Character.isDigit(c)) {
            num = num * 10 + (c - '0');
        }
        if (!Character.isDigit(c) && c != ' ' || i == s.length() - 1) {
            switch (sign) {
                case '+': stack.push(num); break;
                case '-': stack.push(-num); break;
                case '*': stack.push(stack.pop() * num); break;
                case '/': stack.push(stack.pop() / num); break;
            }
            sign = c;
            num = 0;
        }
    }
    
    int result = 0;
    while (!stack.isEmpty()) result += stack.pop();
    return result;
}
```

**思路**：
- 乘除法立即计算
- 加减法入栈最后累加

---

### 二面真题

#### Q1: HTTPS 整个过程？

**参考答案**：

```
客户端                                服务端
   |                                    |
   | -------- Client Hello ----------> |  (支持的加密套件)
   |                                    |
   | <------- Server Hello ----------- |  (选择的加密套件)
   | <------- Certificate ------------- |  (服务端证书)
   |                                    |
   | -------- Client Key Exchange ---> |  (预主密钥，用公钥加密)
   | -------- Change Cipher Spec ----> |  (切换到加密通信)
   | -------- Finished ---------------> |
   |                                    |
   | <------- Change Cipher Spec ----- |  (切换到加密通信)
   | <------- Finished ---------------- |
   |                                    |
   | ======== 加密通信 ================ |
```

**对称 vs 非对称加密**：
- 非对称加密：用于交换对称密钥（RSA、ECDHE）
- 对称加密：用于实际数据传输（AES）

---

#### Q2: 死锁的条件和解决方法？

**四个必要条件**：
1. 互斥：资源只能被一个进程使用
2. 请求保持：持有资源又请求新资源
3. 不剥夺：不能强行剥夺资源
4. 循环等待：存在循环等待链

**解决方法**：
- **预防**：破坏四个条件之一
- **避免**：银行家算法
- **检测**：资源分配图
- **解除**：终止进程、剥夺资源

---

#### Q3: 算法题 - 最长公共连续子串

**参考解答**：
```java
public int longestCommonSubstring(String s1, String s2) {
    int m = s1.length(), n = s2.length();
    int[][] dp = new int[m + 1][n + 1];
    int maxLen = 0;
    
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            if (s1.charAt(i - 1) == s2.charAt(j - 1)) {
                dp[i][j] = dp[i - 1][j - 1] + 1;
                maxLen = Math.max(maxLen, dp[i][j]);
            }
        }
    }
    
    return maxLen;
}
```

**时间复杂度**：O(m × n)
**空间复杂度**：O(m × n)

---

**面试总结**：
- 字节两面连着面，共约 2 小时
- 一面偏八股和基础，二面偏网络和算法
- 面试官引导式提问，体验较好

---


## 2026 最新面经


### 字节跳动一面面经（2026-03-27）

**来源**：[牛客网](https://www.nowcoder.com/enterprise/665/interview)

#### Q1: HTTPS 握手经过几个 RTT（Round Trip Time）？

**参考答案**：

HTTPS 握手过程包括 TCP 三次握手 + TLS 握手，总共需要 **2-3 个 RTT**。

**详细流程**：

```
RTT 1: TCP 三次握手
  客户端 ── SYN ──> 服务端
  客户端 <── SYN+ACK ── 服务端
  客户端 ── ACK ──> 服务端

RTT 2: TLS 握手（以 TLS 1.2 为例）
  客户端 ── ClientHello ──> 服务端
  客户端 <── ServerHello + Certificate + ServerHelloDone ── 服务端

RTT 3: TLS 握手完成
  客户端 ── ClientKeyExchange + ChangeCipherSpec + Finished ──> 服务端
  客户端 <── ChangeCipherSpec + Finished ── 服务端
```

**TLS 1.3 优化**：

TLS 1.3 将握手过程优化为 **1-RTT**：

```
RTT 1: TCP + TLS 合并
  客户端 ── SYN ──> 服务端
  客户端 <── SYN+ACK ── 服务端
  客户端 ── ACK + ClientHello + KeyShare ──> 服务端
  客户端 <── ServerHello + Certificate + Finished ── 服务端
```

**代码示例**：查看 TLS 版本

```bash
# 使用 openssl 查看 TLS 版本
openssl s_client -connect www.example.com:443 -tls1_3

# 查看握手详情
openssl s_client -connect www.example.com:443 -showcerts
```

**追问**：TLS 1.3 相比 TLS 1.2 有哪些改进？

1. 握手从 2-RTT 减少到 1-RTT
2. 移除了不安全的加密算法（如 RSA、CBC）
3. 支持 0-RTT 恢复（有重放攻击风险）

---

#### Q2: Redis 跳表（SkipList）了解吗？为什么 MySQL 不用跳表而用 B+ 树？

**参考答案**：

**跳表结构**：

跳表是在链表基础上改进的「多层」有序链表，能够快速定位数据。

```
Level 3: ────────────────────────►[50]────────────►NULL
                                      │
Level 2: ────►[20]───────────────►[50]────────────►NULL
                  │                   │
Level 1: ────►[20]────►[30]──────►[50]────►[60]──►NULL
                  │         │         │        │
Level 0: ►[10]►[20]►[25]►[30]►[40]►[50]►[60]►[70]►NULL
```

**查找过程**：
- 查找 50：Level 0 → 20 → Level 1 → 50（3 步）
- 时间复杂度：O(log n)
- 空间复杂度：O(n)

**为什么 MySQL 不用跳表**：

| 维度 | B+ 树 | 跳表 |
|------|-------|------|
| 磁盘 IO | 3-4 层可存千万数据 | 层数过高，IO 次数多 |
| 范围查询 | 叶子节点链表，高效 | 需要回到底层遍历 |
| 空间利用率 | 非叶子节点只存键值 | 每层都需要存储指针 |

**核心原因**：B+ 树的高度在 3 层时就能存储千万级别数据，而跳表维护同样数据量会导致层数过高，磁盘 IO 次数增多。

**代码示例**：Redis Zset 跳表实现

```c
// Redis 跳表节点结构
typedef struct zskiplistNode {
    sds ele;                          // 成员对象
    double score;                     // 分值
    struct zskiplistNode *backward;   // 后退指针
    struct zskiplistLevel {
        struct zskiplistNode *forward; // 前进指针
        unsigned long span;            // 跨度
    } level[];                        // 层
} zskiplistNode;
```

**追问**：为什么 Redis 使用跳表而不是红黑树？

1. 实现简单，易于理解和维护
2. 范围查询更高效（只需要遍历底层链表）
3. 内存占用更可控（可以通过调整概率控制层数）
4. 并发友好（锁粒度更细）

---

#### Q3: synchronized 支持重入吗？如何实现的？

**参考答案**：

synchronized 是**可重入锁**，同一个线程可以多次获取同一把锁。

**实现原理**：

每个对象的对象头中存储着锁状态和线程 ID：

```
对象头结构：
┌─────────────────────────────────────────┐
│  Mark Word (64 bits)                    │
├─────────────────────────────────────────┤
│  锁状态 | 线程ID | 锁计数器 | 其他       │
└─────────────────────────────────────────┘

偏向锁状态：
┌─────────────────────────────────────────┐
│  thread:23 | epoch:2 | age:4 | 1 | 01   │
└─────────────────────────────────────────┘

轻量级锁状态：
┌─────────────────────────────────────────┐
│  ptr_to_lock_record:62 | 00             │
└─────────────────────────────────────────┘

重量级锁状态：
┌─────────────────────────────────────────┐
│  ptr_to_heavyweight_monitor:62 | 10     │
└─────────────────────────────────────────┘
```

**重入过程**：

```java
public class ReentrantDemo {
    public synchronized void methodA() {
        methodB();  // 再次获取同一把锁，计数器 +1
    }
    
    public synchronized void methodB() {
        // 执行完毕后，计数器 -1
    }
}
```

**底层实现**：

1. **锁状态 = 0**：锁未被占用，CAS 获取锁，记录线程 ID
2. **锁状态 ≠ 0 且线程 ID = 当前线程**：可重入，计数器 +1
3. **锁状态 ≠ 0 且线程 ID ≠ 当前线程**：阻塞等待

**释放锁**：
- 每次退出 synchronized 方法/代码块，计数器 -1
- 计数器 = 0 时，真正释放锁

**代码示例**：验证可重入

```java
public class ReentrantTest {
    private final Object lock = new Object();
    
    public void outer() {
        synchronized (lock) {
            System.out.println("outer");
            inner();  // 可重入
        }
    }
    
    public void inner() {
        synchronized (lock) {
            System.out.println("inner");
        }
    }
    
    public static void main(String[] args) {
        new ReentrantTest().outer();
    }
}
// 输出：outer inner（不会死锁）
```

**追问**：synchronized 和 ReentrantLock 有什么区别？

| 维度 | synchronized | ReentrantLock |
|------|-------------|---------------|
| 实现层级 | JVM 层面 | API 层面 |
| 锁获取 | 自动 | 手动 lock()/unlock() |
| 公平性 | 非公平 | 可选公平/非公平 |
| 条件变量 | 单一 | 多个 Condition |
| 响应中断 | 不支持 | 支持 lockInterruptibly() |

---

#### Q4: MySQL 间隙锁（Gap Lock）的原理？什么时候会加间隙锁？

**参考答案**：

**间隙锁定义**：

Gap Lock 称为间隙锁，只存在于**可重复读（RR）隔离级别**，目的是解决幻读问题。

**原理**：

间隙锁锁住两个记录之间的间隙，防止其他事务在这个间隙中插入新记录。

```
假设表中有 id = 1, 5, 10 三条记录

间隙锁范围：
(-∞, 1), (1, 5), (5, 10), (10, +∞)

如果事务 A 在 id = 5 上加间隙锁 (1, 5)
则其他事务无法插入 id = 2, 3, 4 的记录
```

**什么时候加间隙锁**：

当使用唯一索引进行等值查询，且**查询的记录不存在**时，会加间隙锁。

```sql
-- 假设表中有 id = 1, 5, 10
-- 事务 A
SELECT * FROM user WHERE id = 3 FOR UPDATE;

-- 此时会加间隙锁 (1, 5)
-- 事务 B 插入 id = 2, 3, 4 都会被阻塞
INSERT INTO user (id, name) VALUES (3, 'test');  -- 阻塞
```

**间隙锁的特点**：

1. 间隙锁之间**兼容**（两个事务可以同时持有同一间隙的间隙锁）
2. 间隙锁目的是**防止插入**，不是防止读取
3. 只存在于可重复读隔离级别

**代码示例**：

```sql
-- 查看当前锁情况
SELECT * FROM performance_schema.data_locks\G;

-- 示例输出
*************************** 1. row ***************************
               ENGINE: INNODB
       ENGINE_LOCK_ID: 140234567890:1070:140234567890
ENGINE_TRANSACTION_ID: 4212345678901234
            THREAD_ID: 45
             EVENT_ID: 12
        OBJECT_SCHEMA: test
          OBJECT_NAME: user
       PARTITION_NAME: NULL
    SUBPARTITION_NAME: NULL
             INDEX_NAME: PRIMARY
OBJECT_INSTANCE_BEGIN: 140234567890
            LOCK_TYPE: RECORD
            LOCK_MODE: X,GAP              -- 间隙锁
          LOCK_STATUS: GRANTED
               LOCK_DATA: 5               -- 锁住 (1, 5) 间隙
```

**追问**：如何避免间隙锁？

1. 使用读提交（RC）隔离级别
2. 使用唯一索引查询存在的记录
3. 使用主键查询

---

#### Q5: 算法题 - 重排链表

**题目**：给定链表 1 → 2 → 3 → ... → n-1 → n，使用 O(1) 空间复杂度使其变为 1 → n → 2 → n-1 → ...

**参考答案**：

**思路**：
1. 找到链表中点
2. 反转后半部分
3. 合并两个链表

**代码实现**：

```java
public void reorderList(ListNode head) {
    if (head == null || head.next == null) return;
    
    // 1. 找中点（快慢指针）
    ListNode slow = head, fast = head;
    while (fast.next != null && fast.next.next != null) {
        slow = slow.next;
        fast = fast.next.next;
    }
    
    // 2. 反转后半部分
    ListNode second = reverse(slow.next);
    slow.next = null;  // 断开
    
    // 3. 合并两个链表
    ListNode first = head;
    while (second != null) {
        ListNode temp1 = first.next;
        ListNode temp2 = second.next;
        
        first.next = second;
        second.next = temp1;
        
        first = temp1;
        second = temp2;
    }
}

// 反转链表
private ListNode reverse(ListNode head) {
    ListNode prev = null, curr = head;
    while (curr != null) {
        ListNode next = curr.next;
        curr.next = prev;
        prev = curr;
        curr = next;
    }
    return prev;
}
```

**示例**：

```
输入：1 → 2 → 3 → 4 → 5
       ↓
中点：3
       ↓
反转：1 → 2 → 3, 5 → 4
       ↓
合并：1 → 5 → 2 → 4 → 3

输入：1 → 2 → 3 → 4 → 5 → 6
       ↓
中点：3
       ↓
反转：1 → 2 → 3, 6 → 5 → 4
       ↓
合并：1 → 6 → 2 → 5 → 3 → 4
```

**复杂度分析**：
- 时间复杂度：O(n)，遍历 3 次
- 空间复杂度：O(1)，只使用常数空间

**追问**：如果要求不能改变原链表结构怎么办？

可以先复制链表再操作，或者使用递归（但空间复杂度变为 O(n)）。

---

## 2026-03-27 字节跳动社招一面真题

**来源**：[牛客网](https://www.nowcoder.com/discuss/739938569828995072)
**时间**：2026-03-27
**背景**：社招 Java 后端

### 一面真题（第一组）

#### Q1: InnoDB 引擎数据结构？

**参考答案**：

**B+ 树结构**：
- 非叶子节点只存索引键值
- 叶子节点存完整数据 + 指向下一叶子节点的指针
- 叶子节点形成双向链表，支持范围查询

**优势**：
1. 树高度低（2-4层），IO次数少
2. 范围查询高效
3. 磁盘友好（节点大小等于页大小）

---

#### Q2: B+ 树中数据怎么存储的？

**参考答案**：

**聚簇索引**：
- 叶子节点存储完整行数据
- 主键即聚簇索引

**非聚簇索引**：
- 叶子节点存储主键值
- 查询需要回表（通过主键再查聚簇索引）

```sql
-- 聚簇索引查询：一次IO
SELECT * FROM user WHERE id = 100;

-- 非聚簇索引查询：回表
SELECT * FROM user WHERE name = '张三';
-- 先查name索引获取id，再查聚簇索引获取完整数据
```

---

#### Q3: B+ 树和 B 树的区别？

**参考答案**：

| 特性 | B 树 | B+ 树 |
|------|------|-------|
| 数据存储 | 所有节点存数据 | 只有叶子节点存数据 |
| 叶子节点 | 独立 | 双向链表连接 |
| 范围查询 | 效率低 | 高效 |
| 查询稳定性 | 不稳定（高度不同） | 稳定（必须到叶子） |

---

#### Q4: binlog 和 redolog？

**参考答案**：

| 特性 | redo log | binlog |
|------|----------|--------|
| 来源 | InnoDB | MySQL Server |
| 格式 | 物理日志 | 逻辑日志 |
| 写入 | 循环写 |追加写 |
| 用途 | 崩溃恢复 | 主从复制、备份 |

**两阶段提交**：
```
1. 写 redo log（prepare状态）
2. 写 binlog
3. 提交 redo log（commit状态）
```

---

#### Q5: Redis 数据结构？

**参考答案**：

| 类型 | 底层实现 | 应用场景 |
|------|---------|---------|
| String | SDS | 缓存、计数器 |
| List | QuickList | 消息队列 |
| Hash | HashTable | 对象存储 |
| Set | HashTable + IntSet | 标签、去重 |
| ZSet | SkipList + HashTable | 排行榜 |

---

#### Q6: ZSet 底层实现？

**参考答案**：

**结构**：
- 跳表（SkipList）：有序，支持范围查询
- HashTable：存储 member → score，O(1) 查询

**跳表优势**：
- 实现简单
- 范围查询高效
- 内存占用可控

```c
typedef struct zset {
    dict *dict;      // member -> score
    zskiplist *zsl;  // 按score排序
} zset;
```

---

#### Q7: 算法题 - 和为 K 的子数组

**题目**：给定数组 nums 和整数 k，返回和为 k 的子数组个数。

**参考答案**：

```java
public int subarraySum(int[] nums, int k) {
    Map<Integer, Integer> prefixSum = new HashMap<>();
    prefixSum.put(0, 1);
    
    int count = 0, sum = 0;
    for (int num : nums) {
        sum += num;
        if (prefixSum.containsKey(sum - k)) {
            count += prefixSum.get(sum - k);
        }
        prefixSum.merge(sum, 1, Integer::sum);
    }
    
    return count;
}
```

**时间复杂度**：O(n)

---

### 一面真题（第二组，被捞）

#### Q1: 接口时延优化？

**参考答案**：

**优化方向**：
1. 数据库优化：索引、SQL优化、分库分表
2. 缓存优化：本地缓存 + 分布式缓存
3. 异步处理：MQ异步化非核心流程
4. 并行调用：CompletableFuture 并行
5. 减少网络IO：合并请求

---

#### Q2: 缓存击穿、穿透、雪崩？

**参考答案**：

| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 穿透 | 查询不存在数据 | 布隆过滤器、空值缓存 |
| 雪崩 | 大量缓存同时失效 | 随机过期、多级缓存 |
| 击穿 | 热点Key过期 | 互斥锁、逻辑过期 |

---

#### Q3: 本地缓存 Caffeine 底层实现？

**参考答案**：

**核心设计**：
- W-TinyLFU 淘汰策略：频率+衰减
- Window缓存区：新数据进入窗口
- Main缓存区：高频数据进入主区

**特点**：
- 高性能：基于ConcurrentHashMap
- 自动淘汰：基于频率和新鲜度
- 异步刷新：后台线程刷新过期数据

---

#### Q4: Redis 缓存失效原理？

**参考答案**：

**过期策略**：
1. 惰性删除：访问时检查
2. 定期删除：每隔一段时间随机检查

**内存淘汰**：
- allkeys-lru：优先淘汰最近最少使用
- volatile-lru：淘汰设置了过期时间的LRU
- allkeys-random：随机淘汰

---

#### Q5: 分库分表如何做？

**参考答案**：

**分库策略**：
- 垂直分库：按业务拆分
- 水平分库：按数据量拆分

**分表策略**：
- 范围分表：按时间/ID范围
- Hash分表：按Hash值
- 一致性Hash：解决扩容问题

---

#### Q6: 分库键的选择？

**参考答案**：

**原则**：
1. 查询频率高的字段
2. 数据分布均匀
3. 避免跨库查询

**常见选择**：
- 用户表：userId
- 订单表：orderId 或 userId
- 日志表：createTime

---

#### Q7: 取余分库分表的问题？

**参考答案**：

**问题**：
1. 扩容困难：需要数据迁移
2. 数据倾斜：某些分片数据多

**解决方案**：
- 一致性Hash：减少迁移量
- 范围+Hash组合：平衡分布和扩容

---

#### Q8: 数据倾斜如何处理？

**参考答案**：

**方案**：
1. 调整分片策略：更均匀的Hash
2. 热点数据单独分片：大用户单独处理
3. 增加分片数：分散数据
4. 冷热分离：历史数据归档

---

#### Q9: ES 数据量过大如何优化？

**参考答案**：

**优化方案**：
1. 分片优化：合理设置分片数
2. 索引优化：减少不必要的索引字段
3. 查询优化：避免深度聚合、使用filter
4. 冷热分离：热数据SSD，冷数据HDD
5. 数据压缩：启用source压缩

---

#### Q10: 算法题 - 连续子数组最大和

**参考答案**：

```java
public int maxSubArray(int[] nums) {
    int maxSum = nums[0], currentSum = nums[0];
    
    for (int i = 1; i < nums.length; i++) {
        currentSum = Math.max(nums[i], currentSum + nums[i]);
        maxSum = Math.max(maxSum, currentSum);
    }
    
    return maxSum;
}
```

---

#### Q11: 算法题 - 最长回文子串

**参考答案**：

```java
public String longestPalindrome(String s) {
    String result = "";
    
    for (int i = 0; i < s.length(); i++) {
        // 偶数长度
        String s1 = expand(s, i, i);
        // 奇数长度
        String s2 = expand(s, i, i + 1);
        
        result = result.length() > s1.length() ? result : s1;
        result = result.length() > s2.length() ? result : s2;
    }
    
    return result;
}

private String expand(String s, int left, int right) {
    while (left >= 0 && right < s.length() && s.charAt(left) == s.charAt(right)) {
        left--;
        right++;
    }
    return s.substring(left + 1, right);
}
```

---

**面试总结**：
- 字节社招一面偏数据库和缓存
- InnoDB、Redis、分库分表是重点
- 算法题中等难度，要求手写
- 被捞后二面会问更多场景问题

---

## 2026-03-29 字节跳动 Java 面经真题

**来源**：[牛客网](https://www.nowcoder.com/discuss/353156517979430912)
**背景**：Java 实习/社招

### Q1: 四种引用类型？

**参考答案**：

| 类型 | 特点 | GC 行为 | 应用场景 |
|------|------|---------|---------|
| **强引用** | 正常引用 | GC 不回收 | 日常对象 |
| **软引用** | SoftReference | 内存不足时回收 | 缓存 |
| **弱引用** | WeakReference | GC 时直接回收 | ThreadLocal Key |
| **虚引用** | PhantomReference | 无法访问对象 | 监控 GC |

**代码示例**：
```java
// 软引用缓存
SoftReference<Image> cache = new SoftReference<>(image);
Image img = cache.get();  // 可能返回 null（已回收）

// 弱引用
WeakReference<User> ref = new WeakReference<>(new User());
// GC 后 ref.get() 返回 null

// 虚引用
PhantomReference<Object> phantom = new PhantomReference<>(obj, queue);
// get() 总是返回 null，通过 queue 监控回收
```

---

### Q2: 协程了解吗？

**参考答案**：

**协程定义**：
- 用户态线程，轻量级
- 由程序自己调度，不依赖操作系统
- 切换开销小，无需内核参与

**特点**：
| 特性 | 线程 | 协程 |
|------|------|------|
| 调度 | OS 内核 | 用户程序 |
| 切换开销 | 大（上下文切换） | 小（几个寄存器） |
| 数量限制 | 几千 | 几十万 |
| 内存占用 | MB 级别 | KB 级别 |

**应用场景**：
- Go goroutine
- Kotlin 协程
- Python async/await

**Java 协程现状**：
- Project Loom（Virtual Thread）
- JDK 21 正式支持

```java
// JDK 21 Virtual Thread
Thread.startVirtualThread(() -> {
    System.out.println("Virtual Thread");
});
```

---

### Q3: 进程间通信方式？

**参考答案**：

| 方式 | 说明 | 特点 |
|------|------|------|
| 管道 | 半双工，父子进程 | 单向传输 |
| FIFO | 命名管道 | 无亲缘关系进程 |
| 共享内存 | 最快 IPC | 需同步机制 |
| 消息队列 | 消息链表 | 异步通信 |
| 信号量 | 同步工具 | 计数器 |
| Socket | 跨机器通信 | 网络通信 |

**共享内存示例**：
```java
// Java 共享内存
MappedByteBuffer buffer = FileChannel.open(path)
    .map(FileChannel.MapMode.READ_WRITE, 0, 1024);
```

---

**面试总结**：
- 四种引用类型要区分清楚
- 协程是趋势，了解 Virtual Thread
- 进程通信要了解多种方式

---

## 2026-03-30 字节跳动社招 Java 一面真题

**来源**：[牛客网](https://www.nowcoder.com/discuss/739938569828995072)
**背景**：社招 Java 后端

### Q1: InnoDB 引擎数据结构？

**参考答案**：

**B+ 树结构**：
- 非叶子节点只存索引键值
- 叶子节点存完整数据 + 指向下一叶子节点的指针
- 叶子节点形成双向链表，支持范围查询

**优势**：
1. 树高度低（2-4 层），IO 次数少
2. 范围查询高效
3. 磁盘友好（节点大小等于页大小）

---

### Q2: B+ 树和 B 树的区别？

**参考答案**：

| 特性 | B 树 | B+ 树 |
|------|------|-------|
| 数据存储 | 所有节点存数据 | 只有叶子节点存数据 |
| 叶子节点 | 独立 | 双向链表连接 |
| 范围查询 | 效率低 | 高效 |
| 查询稳定性 | 不稳定 | 稳定（必须到叶子） |

---

### Q3: binlog 和 redolog 区别？

**参考答案**：

| 特性 | redo log | binlog |
|------|----------|--------|
| 来源 | InnoDB | MySQL Server |
| 格式 | 物理日志 | 逻辑日志 |
| 写入 | 循环写 | 追加写 |
| 用途 | 崩溃恢复 | 主从复制、备份 |

**两阶段提交**：
```
1. 写 redo log（prepare 状态）
2. 写 binlog
3. 提交 redo log（commit 状态）
```

---

### Q4: Redis 数据结构有哪些？

**参考答案**：

| 类型 | 底层实现 | 应用场景 |
|------|---------|---------|
| String | SDS | 缓存、计数器 |
| List | QuickList | 消息队列 |
| Hash | HashTable | 对象存储 |
| Set | HashTable + IntSet | 标签、去重 |
| ZSet | SkipList + HashTable | 排行榜 |

---

### Q5: ZSet 底层实现？

**参考答案**：

**结构**：
- 跳表（SkipList）：有序，支持范围查询
- HashTable：存储 member → score，O(1) 查询

**跳表优势**：
- 实现简单
- 范围查询高效
- 内存占用可控

```c
typedef struct zset {
    dict *dict;      // member -> score
    zskiplist *zsl;  // 按 score 排序
} zset;
```

---

### Q6: 接口时延优化方法？

**参考答案**：

**优化方向**：
1. 数据库优化：索引、SQL 优化、分库分表
2. 缓存优化：本地缓存 + 分布式缓存
3. 异步处理：MQ 异步化非核心流程
4. 并行调用：CompletableFuture 并行
5. 减少网络 IO：合并请求

---

### Q7: Caffeine 底层实现？

**参考答案**：

**核心设计**：
- W-TinyLFU 淘汰策略：频率 + 衰减
- Window 缓存区：新数据进入窗口
- Main 缓存区：高频数据进入主区

**特点**：
- 高性能：基于 ConcurrentHashMap
- 自动淘汰：基于频率和新鲜度
- 异步刷新：后台线程刷新过期数据

---

### Q8: 分库分表如何做？

**参考答案**：

**分库策略**：
- 垂直分库：按业务拆分
- 水平分库：按数据量拆分

**分表策略**：
- 范围分表：按时间/ID 范围
- Hash 分表：按 Hash 值
- 一致性 Hash：解决扩容问题

---

### Q9: 数据倾斜如何处理？

**参考答案**：

**方案**：
1. 调整分片策略：更均匀的 Hash
2. 热点数据单独分片：大用户单独处理
3. 增加分片数：分散数据
4. 冷热分离：历史数据归档

---

### Q10: ES 数据量过大如何优化？

**参考答案**：

**优化方案**：
1. 分片优化：合理设置分片数
2. 索引优化：减少不必要的索引字段
3. 查询优化：避免深度聚合、使用 filter
4. 冷热分离：热数据 SSD，冷数据 HDD
5. 数据压缩：启用 source 压缩

---

### Q11: 算法题 - 和为 K 的子数组

**参考答案**：

```java
public int subarraySum(int[] nums, int k) {
    Map<Integer, Integer> prefixSum = new HashMap<>();
    prefixSum.put(0, 1);
    
    int count = 0, sum = 0;
    for (int num : nums) {
        sum += num;
        if (prefixSum.containsKey(sum - k)) {
            count += prefixSum.get(sum - k);
        }
        prefixSum.merge(sum, 1, Integer::sum);
    }
    
    return count;
}
```

**时间复杂度**：O(n)

---

### Q12: 算法题 - 连续子数组最大和

**参考答案**：

```java
public int maxSubArray(int[] nums) {
    int maxSum = nums[0], currentSum = nums[0];
    
    for (int i = 1; i < nums.length; i++) {
        currentSum = Math.max(nums[i], currentSum + nums[i]);
        maxSum = Math.max(maxSum, currentSum);
    }
    
    return maxSum;
}
```

---

### Q13: 算法题 - 最长回文子串

**参考答案**：

```java
public String longestPalindrome(String s) {
    String result = "";
    
    for (int i = 0; i < s.length(); i++) {
        // 偶数长度
        String s1 = expand(s, i, i);
        // 奇数长度
        String s2 = expand(s, i, i + 1);
        
        result = result.length() > s1.length() ? result : s1;
        result = result.length() > s2.length() ? result : s2;
    }
    
    return result;
}

private String expand(String s, int left, int right) {
    while (left >= 0 && right < s.length() && s.charAt(left) == s.charAt(right)) {
        left--;
        right++;
    }
    return s.substring(left + 1, right);
}
```

---

**面试总结**：
- 字节社招一面偏数据库和缓存
- InnoDB、Redis、分库分表是重点
- 算法题中等难度，要求手写
- 被捞后二面会问更多场景问题

---

## 字节跳动 Java 校招一面二面真题（2026-03-31）

**来源**：[牛客网](https://www.nowcoder.com/discuss/353156517979430912)

**注意**：此面经含"实习"标签，仅供参考面试风格

### 一面真题（80分钟）

#### Q1: Java 集合框架源码？ArrayList、LinkedList 原理？

**参考答案**：

**ArrayList**：
- 底层：动态数组
- 扩容：1.5倍（`int newCapacity = oldCapacity + (oldCapacity >> 1)`）
- 查询：O(1)
- 插入/删除：O(n)（需移动元素）

**LinkedList**：
- 底层：双向链表
- 查询：O(n)
- 插入/删除：O(1)（修改指针）

---

#### Q2: 如何设计一个 HashMap？

**参考答案**：

**设计要点**：
```java
public class MyHashMap<K, V> {
    // 1. 数组存储
    Node<K, V>[] table;
    
    // 2. Hash 函数
    int hash(K key) {
        return key.hashCode() ^ (key.hashCode() >>> 16);
    }
    
    // 3. 定位索引
    int index(int hash, int length) {
        return hash & (length - 1);  // 位运算，要求长度是2的幂
    }
    
    // 4. 扩容
    void resize() {
        Node<K, V>[] newTable = new Node[newCapacity];
        // 重新计算位置...
    }
}
```

---

#### Q3: 线程池执行过程、核心参数、常用线程池？

**参考答案**：

**执行过程**：
```
任务提交 → 核心线程数未满 → 创建核心线程
        → 核心线程已满 → 入队列等待
        → 队列已满 → 创建非核心线程
        → 达到最大 → 执行拒绝策略
```

**常用线程池**：
| 类型 | 特点 | 队列 |
|------|------|------|
| FixedThreadPool | 固定线程数 | 无界 LinkedBlockingQueue |
| CachedThreadPool | 可变线程数 | SynchronousQueue |
| SingleThreadExecutor | 单线程 | 无界队列 |
| ScheduledThreadPool | 定时任务 | DelayedWorkQueue |

---

#### Q4: JVM OOM 如何定位？

**参考答案**：

**定位步骤**：
1. **保留现场**：
   ```bash
   jmap -dump:format=b,file=dump.hprof <pid>
   jstack <pid> > thread.txt
   ```
2. **分析 dump**：MAT、JProfiler 查找大对象
3. **查看 GC 日志**：`-XX:+PrintGCDetails -Xloggc:gc.log`
4. **定位原因**：内存泄漏或对象过大

---

#### Q5: 虚拟机栈可能发生什么错误？

**参考答案**：

| 错误 | 原因 |
|------|------|
| StackOverflowError | 递归过深、栈帧过大 |
| OutOfMemoryError | 线程过多、栈空间不足 |

---

#### Q6: 四种引用类型？

**参考答案**：

| 类型 | GC 行为 | 用途 |
|------|---------|------|
| 强引用 | 不回收 | 正常对象 |
| 软引用 | 内存不足回收 | 缓存 |
| 弱引用 | 随时可能回收 | WeakHashMap |
| 虚引用 | 无法访问 | 跟踪回收 |

---

#### Q7: synchronized 性能为什么提高了？

**参考答案**：

**锁升级过程**：
```
无锁 → 偏向锁 → 轻量级锁 → 重量级锁
```

**优化原因**：
- 偏向锁：无竞争时无同步开销
- 轻量级锁：CAS 替代 Mutex
- 重量级锁：竞争激烈时才使用

---

#### Q8: synchronized 和 Lock 区别？

**参考答案**：

| 特性 | synchronized | Lock |
|------|-------------|------|
| 实现 | JVM 内置 | Java API |
| 释放 | 自动 | 手动 |
| 中断 | 不支持 | 支持 |
| 公平 | 非公平 | 可选 |
| 条件 | 单个 | 多个 |

---

#### Q9: 输入 www.baidu.com 后发生什么？

**参考答案**：

**完整流程**：
1. **DNS 解析**：域名 → IP
2. **TCP 连接**：三次握手
3. **TLS 握手**：HTTPS
4. **HTTP 请求**：发送请求报文
5. **服务器处理**：解析请求、生成响应
6. **HTTP 响应**：返回响应报文
7. **TCP 断开**：四次挥手
8. **浏览器渲染**：解析 HTML、CSS、JS

---

#### Q10: HTTP 报文结构？头部字段？

**参考答案**：

**请求报文**：
```
请求行：GET /path HTTP/1.1
头部：Host, User-Agent, Accept, Content-Type...
空行
消息体
```

**响应报文**：
```
状态行：HTTP/1.1 200 OK
头部：Content-Type, Content-Length, Cache-Control...
空行
消息体
```

---

#### Q11: 进程与线程？协程？

**参考答案**：

| 特性 | 进程 | 线程 | 协程 |
|------|------|------|------|
| 资源 | 独立 | 共享 | 共享 |
| 切换开销 | 大 | 小 | 极小 |
| 通信 | IPC | 共享内存 | 共享内存 |
| 调度 | OS | OS | 用户态 |

---

#### Q12: 死锁条件？如何解决？

**参考答案**：

**四个条件**：
1. 互斥条件
2. 持有并等待
3. 不可剥夺
4. 循环等待

**解决方案**：
1. **预防**：一次性申请所有资源
2. **避免**：银行家算法
3. **检测**：资源分配图
4. **解除**：剥夺资源、终止进程

---

#### Q13: 算法题 - 表达式字符串计算

**参考答案**：

```java
public int calculate(String s) {
    Stack<Integer> stack = new Stack<>();
    int num = 0, result = 0, sign = 1;
    
    for (char c : s.toCharArray()) {
        if (Character.isDigit(c)) {
            num = num * 10 + (c - '0');
        } else if (c == '+') {
            result += sign * num;
            num = 0;
            sign = 1;
        } else if (c == '-') {
            result += sign * num;
            num = 0;
            sign = -1;
        } else if (c == '(') {
            stack.push(result);
            stack.push(sign);
            result = 0;
            sign = 1;
        } else if (c == ')') {
            result += sign * num;
            num = 0;
            result *= stack.pop();  // sign
            result += stack.pop();  // previous result
        }
    }
    return result + sign * num;
}
```

---

### 二面真题（45分钟）

#### Q1: TCP 三次握手四次挥手？

**参考答案**：

**三次握手**：
```
客户端 → 服务器：SYN (seq=x)
服务器 → 客户端：SYN+ACK (seq=y, ack=x+1)
客户端 → 服务器：ACK (seq=x+1, ack=y+1)
```

**四次挥手**：
```
客户端 → 服务器：FIN
服务器 → 客户端：ACK
服务器 → 客户端：FIN
客户端 → 服务器：ACK
```

---

#### Q2: time_wait 和 close_wait 作用？

**参考答案**：

| 状态 | 位置 | 作用 |
|------|------|------|
| time_wait | 主动关闭方 | 确保对方收到 ACK |
| close_wait | 被动关闭方 | 等待应用关闭 |

**大量 time_wait 原因**：
- 短连接频繁建立断开
- HTTP 请求未复用连接

---

#### Q3: 为什么三次握手不能两次？

**参考答案**：

**原因**：
- 防止失效的连接请求到达服务器
- 避免服务器建立无效连接
- 确保双方都能确认对方的接收能力

---

#### Q4: HTTPS 过程？

**参考答案**：

**TLS 握手**：
1. 客户端发送支持的加密算法
2. 服务器返回证书 + 选定算法
3. 客户端验证证书，生成随机数
4. 用公钥加密随机数发送
5. 服务器私钥解密，生成会话密钥
6. 双方用对称密钥通信

---

#### Q5: 进程间通信方式？

**参考答案**：

| 方式 | 特点 |
|------|------|
| 管道 | 单向、父子进程 |
| 消息队列 | 异步通信 |
| 共享内存 | 最快 |
| 信号量 | 同步 |
| Socket | 跨网络 |

---

#### Q6: 算法题 - 最长公共连续子串

**参考答案**：

```java
public String longestCommonSubstring(String s1, String s2) {
    int m = s1.length(), n = s2.length();
    int[][] dp = new int[m + 1][n + 1];
    int maxLen = 0, endIndex = 0;
    
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            if (s1.charAt(i - 1) == s2.charAt(j - 1)) {
                dp[i][j] = dp[i - 1][j - 1] + 1;
                if (dp[i][j] > maxLen) {
                    maxLen = dp[i][j];
                    endIndex = i;
                }
            }
        }
    }
    return s1.substring(endIndex - maxLen, endIndex);
}
```

---

## 2026 社招最新面经（2026-04-04）

**来源**：[牛客网](https://www.nowcoder.com/discuss/739938569828995072)
**背景**：社招一面
**特点**：项目深挖 + 八股 + 算法

### 项目拷打（30min）

涉及 langgraph、agent编排、混合记忆架构等 AI Agent 相关项目：
- agent编排模式、调度机制
- 短期/长期记忆槽位设计
- Token消耗优化：按需加载、渐进式披露
- 子agent动态分配与调度
- 图数据库引入解决的问题

### 八股（15min）

- HashMap底层实现、扩容机制、哈希冲突解决
- HashMap与ConcurrentHashMap区别、线程安全实现
- ReentrantLock公平/非公平实现、AQS原理
- MySQL事务ACID、各特性如何保证
- 最左前缀原则
- Redis ZSET实现、持久化（AOF/RDB）优缺点
- Kafka高可用：生产者/broker/消费者各层面

### 算法题（10min）

#### K个一组反转链表

**题目**：给定链表，每K个节点一组进行反转，返回修改后的链表。

**参考答案**：

```java
public ListNode reverseKGroup(ListNode head, int k) {
    ListNode dummy = new ListNode(0);
    dummy.next = head;
    ListNode prev = dummy, end = dummy;
    
    while (end.next != null) {
        for (int i = 0; i < k && end != null; i++) end = end.next;
        if (end == null) break;
        
        ListNode start = prev.next;
        ListNode next = end.next;
        end.next = null;
        prev.next = reverse(start);
        start.next = next;
        prev = start;
        end = prev;
    }
    return dummy.next;
}

private ListNode reverse(ListNode head) {
    ListNode prev = null, curr = head;
    while (curr != null) {
        ListNode next = curr.next;
        curr.next = prev;
        prev = curr;
        curr = next;
    }
    return prev;
}
```

---

## 字节跳动 Java 社招面经（博客园）（2026-04-05）

**来源**：[博客园](https://www.cnblogs.com/gulihua/articles/18825696)
**背景**：Java 后端社招
**特点**：ES、Redis、Spring 事务、大规模数据处理

### 一、项目相关问题

#### Q1: 介绍一下你最难的项目？

**参考答案**：

用 STAR 法则回答：
- 情境：项目背景、业务场景
- 任务：面临的技术挑战
- 行动：采取的技术方案
- 结果：达到的效果（性能提升、成本降低）

---

#### Q2: 项目中遇到最难的技术难点？

**示例回答**：

亿级别搜索优化：
- 问题：数据量达到亿级，查询慢
- 方案：
  - Redis 缓存热点数据
  - xxl-job 异步任务预热
  - 线程池并发处理
  - 宽表改造减少 JOIN
  - 统计粒度增大

---

### 二、Elasticsearch 相关问题

#### Q1: ES 在项目中有哪些业务场景？

**参考答案**：

| 场景 | 说明 |
|------|------|
| 全文检索 | 商品搜索、文档检索 |
| 日志分析 | ELK 日志收集分析 |
| 实时数据分析 | 监控指标聚合 |
| 推荐系统 | 向量相似度搜索 |

---

#### Q2: ES 数据同步怎么做的？如何保证最终一致性？

**参考答案**：

**同步方案**：
1. **Logstash**：定时同步
2. **Canal**：监听 MySQL binlog
3. **双写**：应用层同时写 DB 和 ES
4. **消息队列**：异步同步

**一致性保障**：
- 幂等性设计：同一数据多次写入结果一致
- 补偿机制：定时任务校验差异
- 死信队列：处理失败的消息

---

#### Q3: ES 内存占用一直 100% 如何解决？

**参考答案**：

**排查步骤**：
1. 检查查询语句是否合理（聚合、深度分页）
2. 检查索引设置（分片数、副本数）
3. 检查字段映射（text vs keyword）

**解决方案**：
1. **增加节点**：水平扩展
2. **减少副本**：降低内存占用
3. **优化查询**：避免深度聚合
4. **冷热分离**：热数据 SSD，冷数据 HDD
5. **内存限制**：设置 `indices.fielddata.cache.size`

---

#### Q4: ES 相较于 MySQL 有哪些优点？

**参考答案**：

| 特性 | MySQL | Elasticsearch |
|------|-------|--------------|
| 全文检索 | 弱 | 强（倒排索引） |
| 分布式 | 需要中间件 | 原生支持 |
| 扩展性 | 垂直扩展为主 | 水平扩展容易 |
| 查询灵活性 | SQL 强大 | DSL 灵活 |
| 事务 | 支持 | 不支持 |

---

### 三、Redis 相关问题

#### Q1: Redis 数据备份的原理？

**参考答案**：

**RDB（快照）**：
- 定时将内存数据保存到磁盘
- 文件小，恢复快
- 可能丢失最后一次快照后的数据

**AOF（日志）**：
- 记录每个写操作命令
- 数据更安全
- 文件大，恢复慢

---

#### Q2: AOF 有哪些机制？

**参考答案**：

**刷盘策略**（appendfsync）：
| 策略 | 说明 | 安全性 | 性能 |
|------|------|--------|------|
| always | 每个命令刷盘 | 最高 | 最低 |
| everysec | 每秒刷盘 | 高 | 高 |
| no | 由 OS 决定 | 低 | 最高 |

**AOF 重写**：
- 压缩 AOF 文件
- 后台执行，不阻塞服务
- 触发条件：`auto-aof-rewrite-percentage`、`auto-aof-rewrite-min-size`

---

#### Q3: 只有 AOF 如何恢复数据？

**参考答案**：

```bash
# 1. 关闭 Redis
redis-cli shutdown

# 2. 删除 RDB 文件（如果有）
rm dump.rdb

# 3. 启动 Redis，自动加载 AOF
redis-server redis.conf
```

---

### 四、Spring 事务相关问题

#### Q1: 讲一下 Spring 事务原理？

**参考答案**：

**核心机制**：
- **AOP 代理**：通过代理对象管理事务
- **PlatformTransactionManager**：事务管理接口
- **TransactionDefinition**：事务定义（隔离级别、传播行为）

**执行流程**：
```
方法调用 → 代理对象拦截 → 开启事务 → 执行目标方法 → 提交/回滚事务
```

---

#### Q2: 事务的传播机制？

**参考答案**：

| 传播行为 | 说明 |
|---------|------|
| REQUIRED（默认） | 有事务就加入，没有就新建 |
| REQUIRES_NEW | 总是新建事务 |
| SUPPORTS | 有事务就加入，没有就非事务执行 |
| NOT_SUPPORTED | 非事务执行 |
| MANDATORY | 必须在事务中执行，否则抛异常 |
| NEVER | 不能在事务中执行，否则抛异常 |
| NESTED | 嵌套事务 |

---

#### Q3: 事务中 A 调用 B 方法，B 方法数据库连接资源已占满，会发生什么？

**参考答案**：

**结果**：
- 抛出异常
- 整个事务回滚

**原因**：
- 同一事务共享同一个连接
- B 获取连接失败 → 抛异常 → 事务回滚

---

### 五、大规模数据处理策略

#### Q1: 数据库数据量达到几亿级别如何处理？

**参考答案**：

**解决方案**：
1. **分库分表**：水平拆分
2. **读写分离**：主库写，从库读
3. **冷热分离**：热数据在线，冷数据归档
4. **缓存**：Redis 缓存热点数据
5. **ES**：复杂查询走 ES

---

#### Q2: ES、MySQL 深度分页如何解决？

**参考答案**：

**MySQL 深分页**：
```sql
-- 问题
SELECT * FROM user LIMIT 100000, 10;

-- 解决方案 1：游标分页
SELECT * FROM user WHERE id > 100000 LIMIT 10;

-- 解决方案 2：延迟关联
SELECT * FROM user 
INNER JOIN (SELECT id FROM user LIMIT 100000, 10) t 
ON user.id = t.id;
```

**ES 深分页**：
```json
// 问题：from + size > 10000 报错
// 解决方案 1：scroll（快照查询）
// 解决方案 2：search_after（游标查询）
{
  "size": 10,
  "search_after": [last_sort_value],
  "sort": [{"id": "asc"}]
}
```

---

#### Q3: 数据预热怎么做？如何保证一致性？

**参考答案**：

**预热策略**：
1. **启动预热**：应用启动时加载热点数据到缓存
2. **定时预热**：xxl-job 定时刷新
3. **渐进式预热**：按需加载

**一致性保障**：
- 缓存过期时间
- 双写一致性：先更新 DB，再删缓存
- 消息队列异步同步

---

**面试总结**：
- ES 场景、数据同步、内存优化是重点
- Redis 持久化机制要熟悉
- Spring 事务原理和传播行为常考
- 大规模数据处理要有实战经验

---

## 2026-04-06 社招面经补充真题

**来源**：[牛客网](https://www.nowcoder.com/discuss/353156058099163136)

> 特点：三年经验社招，系统设计 + 算法 + 架构

### Q1: 算法 - 返回数字数组的排序值

**题目**：返回一个数字数组的排序值，比如数据 [6,2,5,0] 的返回是 [4,2,3,1]

**参考答案**：

```java
public int[] getRank(int[] nums) {
    int n = nums.length;
    int[] result = new int[n];
    
    // 创建带索引的数组
    int[][] indexed = new int[n][2];
    for (int i = 0; i < n; i++) {
        indexed[i] = new int[]{nums[i], i};
    }
    
    // 按值排序
    Arrays.sort(indexed, (a, b) -> a[0] - b[0]);
    
    // 计算排名（相同值排名相同）
    int rank = 1;
    for (int i = 0; i < n; i++) {
        result[indexed[i][1]] = rank;
        if (i < n - 1 && indexed[i][0] != indexed[i + 1][0]) {
            rank++;
        }
    }
    
    return result;
}
```

---

### Q2: 系统设计 - 加载 100G 数据同时处理请求

**题目**：一个系统需要加载 100G 的数据，在这期间还要能正常处理外部请求

**参考答案**：

**核心思路**：异步加载 + 流量控制

```java
// 1. 分片加载
public class DataLoader {
    private volatile boolean loaded = false;
    private ExecutorService executor;
    
    public void init() {
        executor.submit(() -> {
            // 分片加载，每片 1G
            for (int i = 0; i < 100; i++) {
                loadShard(i);
                Thread.sleep(100);  // 让出 CPU 处理外部请求
            }
            loaded = true;
        });
    }
    
    // 2. 请求处理时检查加载状态
    public Response handle(Request req) {
        if (!loaded && needLoadedData(req)) {
            // 返回降级数据或等待
            return fallback(req);
        }
        return process(req);
    }
}
```

**关键点**：
1. 分片加载，避免一次性占用大量内存
2. 异步加载，不阻塞请求处理
3. 限流控制，防止加载过程影响正常请求
4. 降级策略，部分数据未就绪时返回降级结果

---

### Q3: 虚拟内存是什么？

**参考答案**：

**定义**：虚拟内存是操作系统提供的一种内存管理技术，使程序认为自己拥有连续的内存空间，实际物理内存可能不连续，甚至部分数据在磁盘上。

**核心机制**：
1. **分页**：将虚拟内存和物理内存分成固定大小的页
2. **页表**：映射虚拟页到物理页
3. **缺页中断**：访问的页不在物理内存，触发中断从磁盘加载

**优势**：
- 进程隔离：每个进程有独立虚拟地址空间
- 内存保护：防止进程间相互访问
- 内存扩展：可使用大于物理内存的空间

---

### Q4: 什么是内核态？

**参考答案**：

**内核态 vs 用户态**：

| 特性 | 内核态 | 用户态 |
|------|--------|--------|
| 权限 | 最高 | 受限 |
| 可执行指令 | 所有指令 | 非特权指令 |
| 访问内存 | 所有内存 | 自己的地址空间 |
| 发生时机 | 系统调用、中断、异常 | 用户程序运行 |

**用户态 → 内核态**：
1. 系统调用（read、write）
2. 中断（IO 完成、时钟）
3. 异常（缺页、除零）

---

### Q5: 算法 - 正数数组统计出现次数（O(n) 时间 O(1) 空间）

**题目**：一个正数数组，长度为 N，且数组元素 < N，统计每个正数出现的次数，要求时间复杂度 O(n)，空间复杂度 O(1)

**参考答案**：

```java
public void countFrequency(int[] nums) {
    int n = nums.length;
    
    // 用原数组存储频率，利用元素 < n 的特性
    for (int i = 0; i < n; i++) {
        nums[nums[i] % n] += n;
    }
    
    // 输出结果
    for (int i = 0; i < n; i++) {
        System.out.println(i + " 出现 " + nums[i] / n + " 次");
    }
}
```

**原理**：利用 `nums[i] % n` 作为索引，加上 `n` 来记录出现次数

---

### Q6: 算法 - 树的左视图

**参考答案**：

```java
public List<Integer> leftView(TreeNode root) {
    List<Integer> result = new ArrayList<>();
    if (root == null) return result;
    
    Queue<TreeNode> queue = new LinkedList<>();
    queue.offer(root);
    
    while (!queue.isEmpty()) {
        int size = queue.size();
        for (int i = 0; i < size; i++) {
            TreeNode node = queue.poll();
            if (i == 0) result.add(node.val);  // 每层第一个
            if (node.left != null) queue.offer(node.left);
            if (node.right != null) queue.offer(node.right);
        }
    }
    
    return result;
}
```

---

### Q7: 系统设计 - 微信朋友圈 / 微博

**参考答案**：

**微信朋友圈**：
1. **发布**：写入 DB，推送给好友的时间线
2. **读取**：拉取好友列表，按时间排序
3. **点赞/评论**：计数器 + 消息通知

**微博（高并发场景）**：
1. **推模式**：发布时推给所有粉丝（粉丝少时）
2. **拉模式**：读取时从关注人拉取（关注人多时）
3. **推拉结合**：大 V 推，普通用户拉

**核心差异**：
- 朋友圈：强关系，好友数量有限，推模式可行
- 微博：弱关系，粉丝可达百万，需要推拉结合

---

### Q8: 同步/异步、并行/串行

**参考答案**：

| 概念 | 说明 |
|------|------|
| 同步 | 调用后等待结果返回 |
| 异步 | 调用后立即返回，结果通过回调获取 |
| 并行 | 多个任务同时执行（多核） |
| 串行 | 多个任务依次执行 |

**示例**：
```java
// 同步
String result = httpClient.get(url);  // 阻塞等待

// 异步
httpClient.asyncGet(url, response -> {
    // 回调处理
});

// 并行
CompletableFuture.allOf(
    CompletableFuture.runAsync(task1),
    CompletableFuture.runAsync(task2)
).join();
```

---

### Q9: 单例模式有什么缺点？

**参考答案**：

**缺点**：
1. **违反单一职责**：既管理创建又管理业务
2. **难以测试**：全局状态，测试需要重置
3. **隐藏依赖**：使用者不知道单例依赖哪些资源
4. **并发问题**：懒加载需要同步，影响性能
5. **扩展困难**：无法继承、难以替换实现

**替代方案**：
- 依赖注入（Spring Bean 默认单例）
- 工厂模式

---

### Q10: 为什么要做系统拆分？

**参考答案**：

**原因**：
1. **解耦**：降低系统复杂度
2. **独立部署**：小改动不需要发布整个系统
3. **扩展性**：按需扩展瓶颈服务
4. **团队协作**：不同团队负责不同服务
5. **容错**：一个服务挂了不影响其他

**拆分原则**：
- 按业务领域拆分
- 高内聚、低耦合
- 避免循环依赖

---

### Q11: AOP 和 OOP 的区别？

**参考答案**：

| 特性 | OOP | AOP |
|------|-----|-----|
| 关注点 | 业务逻辑 | 横切关注点 |
| 实现方式 | 继承、封装、多态 | 动态代理、织入 |
| 适用场景 | 核心业务 | 日志、事务、权限 |

**代码对比**：
```java
// OOP：业务逻辑和日志混在一起
public void transfer() {
    log.info("开始转账");
    // 转账逻辑
    log.info("转账完成");
}

// AOP：业务逻辑和日志分离
@Log
public void transfer() {
    // 只关注转账逻辑
}
```

---

**面试总结**：
- 系统设计题要考虑高并发场景
- 算法题要求时间空间复杂度优化
- 单例、AOP 等设计模式要深入理解

---

[返回目录](../README.md)

